...
@export var ability_text = "MISSILE SPEED +10%"
...
func randomize_values() -> void:
	...
	cost = selected_value
	cost_text = str(cost)
	ability_text = "MISSILE SPEED +" + str(cost_text) + "%"
...
func activate() -> void:
	# Access the ModManager node to modify ProjectileSpeedMod and Money
	var mod_manager = get_tree().root.get_node("Level/ModManager")
	mod_manager.ProjectileSpeedMod *= 1 + cost / 100.0
